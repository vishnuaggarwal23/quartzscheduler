...
let formSubmitHandler = function (form) {
    ...
    $.ajax({
        url: $(form).data('createNewCronTriggerUri').trim(),
        type: HTTP_POST,
        contentType: APPLICATION_JSON,
        dataType: JSON_DATA_TYPE,
        data: JSON.stringify({
        ...
        complete: function (response) {
            ...
            if (response) {
                if (is2xxResponseCode(response.status) && response.responseJSON && response.responseJSON.triggerDetails) {
                    showSuccessMessage("Cron Trigger is successfully created");
                    goForError = false;
                }
            }
            if (goForError) {
...
const handleCreateCronTrigger = function () {
    $('form#createCronTriggerForm').validate({
...
$(document).ready(function () {
    ...
    $('form#createCronTriggerForm input').keypress(function (e) {
        if (e.which === 13) {
            if ($('form#createCronTriggerForm').validate().form()) {
                $('form#createCronTriggerForm').submit();
            } else {
                return false;
            }
        }
    });
    handleCreateCronTrigger();
});
